...
import (
	...
	extract "Streamline/internal/extract"
...
func main() {
        ...
        for w := 0; w < *workers; w++ {
            ...
            go func() {
                ...
                for f := range jobs {
                    ...
                    if !extract.IsPathWithinBase(*outDir, targetPath) {
                        log.Printf("[ERROR] Illegal file path: %s", targetPath)
                        continue
                    }
                    if err := extract.ExtractFile(f, targetPath); err != nil {
    ...
    if *boost {
        for _, f := range zr.File {
            if !extract.ShouldExtract(f.Name, *include, *exclude) {
